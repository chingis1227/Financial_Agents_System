...
Expected Full Cycle artifact: `financial_statement_analysis.md`. The skill remains the method layer; the owning agent or workflow owns role, boundary, status, and handoff publication.

When this skill runs as part of an equity Full Cycle, it provides financial-quality, cash-flow, balance-sheet, dilution, and accounting method handoffs for Equity, Valuation, Risk, and IC. It must not invent missing line items or replace valuation. Missing or stale statements constrain the method output to `Limited` or `Blocked`.

...
## Russian clean memo language

Russian `investment_report.md` should use natural investment Russian. Do not leak English runtime labels such as Limited, Blocked, gate, handoff, agent, source tier, provider failed, paywall, premium data, runtime, or module status. Translate constraints into Russian investment meaning: ???? ???????????, ???????????????? ? ??????????, ???????? ?????????, ??????????? ?? ?????? ??????, ????? ???? ?? ?????????.

...
When this skill runs as part of a fixed-income large workflow, it owns instrument identity, yield/spread, duration, credit, structure, covenant/call/liquidity, and downside-compensation handoff. If issuer, maturity, seniority, currency, coupon, ISIN/CUSIP, or price/yield context is missing, identity or valuation support is `Limited` or `Blocked`.

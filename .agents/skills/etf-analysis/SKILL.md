...
When this skill runs as part of an ETF or fund Full Cycle, it owns wrapper quality, exposure purity, fees, liquidity, holdings, tracking, and implementation handoff. It must not issue final IC Action or exact allocation. If wrapper or holdings data is stale or incomplete, mark implementation quality `Limited` or `Blocked`.

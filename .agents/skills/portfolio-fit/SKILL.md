...
Expected Full Cycle artifact: `portfolio_fit.md`. The skill remains the method layer; the owning agent or workflow owns role, boundary, status, and handoff publication.

When this skill runs as part of a Full Cycle, it provides generic role fit and user-specific portfolio-fit method output when context exists. Missing portfolio data must not stop asset analysis, but it makes Portfolio Fit `Limited / not personalized` and prevents personalized final IC Action. Exact allocation instructions remain prohibited.

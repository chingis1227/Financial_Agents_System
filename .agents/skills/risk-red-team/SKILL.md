...
Expected large-workflow artifact: `risk_red_team.md`. The skill remains the method layer; the owning agent or workflow owns role, boundary, status, and handoff publication.

When this skill runs as part of a large workflow, it provides thesis challenge, failure paths, bear-case pressure tests, and risk-gate method handoff. It may fail the risk gate but must not issue buy/sell/hold action. If thesis, valuation, or evidence inputs are incomplete, mark the risk review `Limited` or `Blocked` and state what IC cannot conclude.

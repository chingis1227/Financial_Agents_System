...
- If the user asks for a `QUICK:` / fast / short / quick-take answer, ask exactly 3 relevant questions in one block, wait for the user's next message, then route to chat-only Quick Take / Preliminary; do not create `investment_report.md` or `audit`, and do not present it as final IC Action.
...
- Subagent execution must be real: claim agent workflow execution only when relevant subagents are actually spawned and return structured handoffs or artifact-equivalent summaries. If subagent spawning tooling is unavailable or no subagents are actually spawned, record a production blocked state when subagents are unavailable only in audit metadata, mark user-facing output Blocked, and do not imply agent workflow execution.
...
Required large-workflow artifacts and modules:
- User-facing saved report: `investment_report.md`.
- Audit metadata: `audit\run_metadata.md`, `audit\sources.md` by default, `audit\intake.md`, and one handoff/report for every actually run subagent or module.
...
| Final IC synthesis | Investment Committee Agent | Evidence Collection; Investment Committee Synthesis | Valuation & Expectations; Risk / Red Team; Portfolio Fit; other material specialists | Internal `final_investment_memo.md` only when gates are complete; saved user-facing large-workflow output remains `investment_report.md` |
...
| P4-RTE-01-13 | User says "run all agents". | Interpret as all relevant workflow agents, not literally every system agent; use an agent workflow with relevant spawned subagents only when they are actually spawned. | Workflow audit plan records execution mode, included/excluded agents or modules, and handoff requirements; ordinary chat shows these only if requested; Complete or positive IC Action Status only when all required gates pass. | Exhaustive panel may be offered, but irrelevant agents remain excluded, subagents must not be claimed unless actually spawned, and IC gates still govern action. |

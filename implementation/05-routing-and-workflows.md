...
- Subagent execution must be real: claim agent workflow execution only when relevant subagents are actually spawned and return structured handoffs or artifact-equivalent summaries. If subagent spawning tooling is unavailable or no subagents are actually spawned, record a non-spawned-subagent fallback only in audit metadata, mark user-facing output Limited, and do not imply agent workflow execution.

...
|---|---|---|
| Pareto Gate | Pass | 14 / 14 synthetic fixtures have auditable structural assertions and no safety failure, including AGENT workflow default for concrete-asset action requests. |
| Full Regression | Pass | 13 / 13 canonical scenario families covered and safe by documented route/gate behavior, including concrete-asset internal full workflow action requests. |
...
| Session 09 Runtime Workflow QA | Pass | S09 fixtures now cover Non-spawned-subagent and Spawned-subagent modes, Microsoft, QQQ vs SCHG, gold setup now, BTC 3-year, fixed-income ambiguity, handoff artifacts, no premature IC Action, Portfolio Fit limitation, and stale-evidence handling. |
...
|---|---|---|---|---|---|---|---|
| Pareto Gate | P10-PAR-01 | "Should I buy Acme Corp?" with no position, horizon, objective, or risk profile. | Concrete-asset action routes to `AGENT:` / internal full workflow unless explicitly `QUICK:` / short; missing portfolio context limits Portfolio Fit / IC Action and no final IC Action is allowed. | `implementation/00-master-rules.md`, `implementation/05-routing-and-workflows.md`, and `AGENTS.md` now require `AGENT:` / internal full workflow by default for concrete-asset action requests and `Decision-Prep Memo` when portfolio context is missing. | Pass | Pass | None |
...
### Golden fixture: Microsoft internal full workflow Russian prompt
...
Session 09 hardening adds explicit runtime-behavior fixtures for the new Equity large workflow and Agent workflow with spawned subagents. These rows are structural / behavioral acceptance checks. They do not score investment correctness and do not claim that live spawned-subagent smoke testing was performed; live Microsoft agent workflow execution is reserved for Session 10.
...
|---|---|---|---|---|---|---|---|---|---|
| Runtime Workflow | S09-RUNTIME-01 | Microsoft, no portfolio, 3+ year horizon. | Non-spawned-subagent Equity internal full workflow with Execution mode, Runtime Execution Plan, required module statuses, mandatory handoffs, Portfolio Fit Limited, `decision_prep_memo.md`, and no final positive IC Action. | `workflows/equity_full_cycle.md`, `workflows/handoff_artifact_standard.md`, QA Scenario 1A, and the golden Microsoft fixture require this behavior. | Pass | Pass | None | None | Use as the non-spawned-subagent golden fixture in future regression runs. |
| Runtime Workflow | S09-RUNTIME-02 | Explicit spawned-subagent Microsoft spawned-subagent workflow with subagents. | Spawned-subagent mode only if subagents are actually spawned; list spawned/skipped relevant agents; consume only structured handoffs; missing portfolio context still prevents final positive IC Action. | Runtime runbook and Session 09 QA rows now make false subagent spawning, missing spawned-agent list, and unstructured handoffs acceptance failures. | Pass | Pass | None | None | Session 10 should perform the live spawned-subagent smoke test and record actual spawned-agent handoff summaries. |

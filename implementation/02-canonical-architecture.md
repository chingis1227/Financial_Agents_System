...
|---|---|
| User asks for buy / sell / hold immediately. | For a concrete-asset investment action request, route to `AGENT:` / the internal full workflow unless the user explicitly asks for `QUICK:` / short / fast / quick take / preliminary. Final `IC Action` belongs only to the Investment Committee after required gates. |
| User omits horizon, objective, risk tolerance, or position context. | If asset identity and route are clear, continue the appropriate workflow: concrete-asset investment-action requests route to `AGENT:` / the internal full workflow, with Portfolio Fit / IC Action marked `Limited` when personal context is missing. Ask only for blocking context, such as ambiguous instrument identity or exact personal trade instructions that cannot be framed safely without context. Do not issue final `IC Action` until decision-critical context is sufficient. |
...
| User explicitly asks for a fast answer even though a full decision is needed. | Provide `Quick Take / Preliminary` only, preserve missing gates and boundaries, and offer `AGENT:` / gated IC workflow for final decision support. |

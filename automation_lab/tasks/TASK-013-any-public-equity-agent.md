...
- Historical fixture tests and live smokes pass or downgrade with recorded reasons.
...
- Production live execution ignores deprecated timeout env vars such as `FA_AUTOMATION_CODEX_SDK_TIMEOUT_SECONDS`, `FA_AUTOMATION_CODEX_SDK_TIMEOUT_EQUITY_AGENT`, and `FA_AUTOMATION_AGENT_TOTAL_TIMEOUT_SECONDS`; runs are unbounded by fixed workflow caps and end only when required specialists complete or an external hard failure blocks execution. `FA_AUTOMATION_AGENT_MAX_PARALLEL_SPECIALISTS` still controls concurrency only.
- Failed live specialist attempts remain in `attempted_subagents` only and do not appear in `actual_subagents_run`; production does not impose fixed timeout caps.
...
Required historical fixture smokes:
...
.\.venv\Scripts\python.exe fa_automation.py agent-run --prompt "AAPL for 3 years" --continue-with-baseline --mode live
.\.venv\Scripts\python.exe fa_automation.py validate-agent-run
.\.venv\Scripts\python.exe fa_automation.py agent-run --prompt "NVDA for 3 years" --continue-with-baseline --mode live
.\.venv\Scripts\python.exe fa_automation.py validate-agent-run
.\.venv\Scripts\python.exe fa_automation.py agent-run --prompt "GOOGL for 3 years" --continue-with-baseline --mode live
.\.venv\Scripts\python.exe fa_automation.py validate-agent-run
...
```powershell
# Deprecated timeout env vars are ignored in production live mode
# Full workflow timeout caps are disabled
...
- Historical fixture smokes passed and validated for: MSFT, AAPL, NVDA, GOOGL, BABA, ASML.AS.
- Initial historical fixture smoke results were saved to `runs/task013_mock_validation.json`; post-review-1 historical fixture smoke results were saved to `runs/task013_mock_validation_review1.json`; numeric non-US smoke results for `7203.T` and `9988.HK` were saved to `runs/task013_numeric_non_us_mock_smokes.json`.
...
- Historical fixture smokes: passed.
...
- `validate-agent-run`: passed for successful/limited historical fixture and live smoke packages.

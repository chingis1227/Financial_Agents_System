...
def _reader_summary(state: dict[str, Any], ic: dict[str, Any]) -> str:
    if state.get("final_status") == "Blocked":
        return "The available material is not enough for a decision-oriented conclusion. The report identifies what needs to be collected or verified before the view can be used."
...
def _valuation_summary(valuation_status: str) -> str:
    ...
    return "A source-backed valuation and expectations view is still needed before this report can support a final action-oriented decision."

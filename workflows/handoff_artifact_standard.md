...
In `Production Blocked - subagents unavailable`, the main Codex session may create only blocker/audit records that explain which required subagents were unavailable, failed, or missing. It must not create substitute module handoff summaries as if the live agent workflow had run. If an `AGENT:` request cannot actually spawn and complete relevant required subagents, the user-facing saved report or chat output must be marked `Blocked` and must not claim agent workflow execution. In `Agent workflow with spawned subagents`, spawned subagents must return separate structured handoff artifacts or clearly labeled artifact-equivalent summaries with the same fields, saved into the audit package before IC synthesis consumes them.
...
- `audit\run_metadata.md` — execution mode, runtime plan, included/excluded modules, actually spawned agents, blocking reason if route-relevant subagents cannot run, canonical internal artifact type, module statuses, and gate status.
...
Ordinary chat shows only the reader-facing report and the `investment_report.md` path. The audit path and technical materials are shown only when explicitly requested. When portfolio context is missing, `investment_report.md` uses a plain `Portfolio role` / `Портфельная роль` section; `Portfolio Fit: Limited / not personalized`, General Portfolio Role Mode, missing gates, execution mode, route, module status, and handoff metadata remain audit-only.
...
| `portfolio_fit.md` | Portfolio Fit Agent / skill | Output status; portfolio-context evidence status; personalization status; role-fit support. When user portfolio context is absent, audit records General Portfolio Role Mode and `Portfolio Fit: Limited / not personalized`. | Missing current position, objective, horizon, risk tolerance, exposure, constraints, overlap, sizing, tax/custody/access details when material. | To IC as portfolio role and personalization gate. Must state `Boundary: Not an IC Action` and must not provide exact trade instructions or exact allocation. Reader-facing `investment_report.md` summarizes this as general Portfolio role, not as a Portfolio Fit failure. |

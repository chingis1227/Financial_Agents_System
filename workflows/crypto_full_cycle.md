...
route: Crypto asset / crypto-linked exposure
used_by:
...
Use this workflow when the user asks whether BTC, ETH, or another crypto asset is investable, attractive, suitable, or worth holding over a stated horizon. If identity, wrapper, structure, currency, or instrument terms materially change the route, ask the minimum clarifying question before decision-critical analysis.
...
| `crypto-agent` | Required | Produces or validates its route-specific handoff; must not exceed its decision boundary. |
...
|---|---|---|
| `etf-agent` | Conditional | Implementation is through a spot crypto ETF or other fund wrapper. |
| `commodity-agent` | Conditional | User asks BTC vs gold or digital-gold framing. |
| `equity-agent` | Conditional | User asks about crypto equities, miners, exchanges, or public-company exposure. |
| `fixed-income-agent` | Conditional | User compares crypto to cash, T-bills, yield products, or credit exposure. |
| `sector-industry-analysis-agent` | Conditional | User asks for crypto-sector structure or thematic mapping. |
...
Crypto analysis must cover network use, token economics, liquidity, regulation, custody/security, implementation quality, drawdown behavior, leverage/yield hazards, and crypto-native valuation or expectations rather than equity-style DCF.

...
Use this workflow when the user compares ETF/fund wrappers, asks which fund is better for an exposure, or evaluates an ETF for capital allocation. If identity, wrapper, structure, currency, or instrument terms materially change the route, ask the minimum clarifying question before decision-critical analysis.
...
| `sector-industry-analysis-agent` | Required | Produces or validates its route-specific handoff; must not exceed its decision boundary. |
...
|---|---|---|
| `fixed-income-agent` | Conditional | ETF owns bonds or duration/credit is the dominant exposure. |
| `commodity-agent` | Conditional | ETF is a commodity wrapper such as gold, oil, or broad commodities. |
| `crypto-agent` | Conditional | ETF is a crypto wrapper. |
| `equity-agent` | Conditional | ETF holdings require a company-level deep dive. |
| `structural-winners-discovery-agent` | Conditional | User asks for candidate discovery rather than ETF comparison. |
...
ETF analysis must cover wrapper identity, index or methodology, holdings, concentration, overlap, fees, liquidity, tracking, tax/access limits when material, and whether the user is asking for exposure analysis or a personal portfolio action.
